...
\ac{vm} boot times, and scaling/scaleout  policies. The data center is therefore % scaleout ?
...
The  study conducted  in this  paper  is built upon an  original comparison  between %JG: Genuine?
...
The provisioning and scheduling strategies of Schlouder are bi-objective: (1) % bi-objective?
...
\begin{figure*}
	...
	\caption{Distributions of makespan and BTU count across the OMSSA and 
	  Montage real-life executions described in Table~\ref{tab:nbruns}}
	\label{fig:realbrs}
\end{figure*}
%JG: vérifier à l'impression N/B
...
corresponding simulations inputted with \emph{effective runtime} from reality. 
...
Then, we used those traces to  generate our \ac{mcs}
input \acp{rv} using the method we will describe in Section~\ref{sec:im}.

Finally, we compared the makespan  and \ac{btu}  distributions of  the \ac{mcs}  to the
...
a group is defined by a unique couple <application, strategy>. %J'ai exclu la plateforme vu qu'on a qu'une 
...
from 12800 to 13400 s for <OMSAA, ASAP> % mettre les nombres exacts et ajouter les autres.
...
perturbation  level  for  all  job.   The perturbation  level is denoted as $P$
...
\begin{equation}
	p = \mathcal{U}(-P,+P)
	\label{eq:r}
\end{equation} % JG: J'ai rien vérifié
where $r_j^n$ is the $n$th runtime observation for job $j$, $\bar{R_j}$ is the
average of all observed runtimes for job $j$, and $\mathcal{U}$ an uniform distribution. %JG: mettre avant, dans un bloc modèle
...
\begin{table}
	...
	\caption{Makespan and BTU capture rate depending on confidence interval
          (CI) for a 10\% perturbation level}\label{tab:fit} %JG: uniformiser CI avec la caption d'avant ?
	\begin{tabular}{llccc}
		\toprule
...
executions, as in  fig.~\ref{fig:realbrs}, are shown as  non-filled curves.   On the %Faire des non-filled curves dans realbrs aussi
\ac{btu} count graphs, the  left bar represents
...
does   the  prediction   change   when  increasing   this  perturbation   level. %Ajouter ici ce que ça peut apporter comme information, voir plus bas. Dire aussi que dans toute cette partie, on se concentre uniquement sur ASAP.
...
using  the average  runtimes without  any randomization.   Comparing the
interval of  perturbed simulations  to this unperturbed  simulation allows  us to %C'ets cette phrase qu'il faut remonter, et ajouter des infos: à quoi sert de voir cet effet ?
...
the intervals naturally get wider, but the average makespan, which is by definition the
center of the capture interval, also increases. This happens to the 
...
from 3\% (7 minutes) to 10\% (almost 24 minutes). This result is two-fold.

Firstly, this demonstrates that the perturbation level can
...
widely. However, the same observation on <ASAP,Montage> (third subfigure) shows that when
...
level. This behaviour is explained by the nature of workflows,
in which jobs have to wait for their latest dependencies resolution to tun: 
