...
perspective of this work is to apply the method to production clouds. This
implies to automatize the process we described as best as possible, especially
regarding the capture of job runtimes. Our second perspective, now that we are
confident in our ability application executions, is to move away from the
applications in our archive and using our enriched simulation to study the
interactions between scheduling and workflow. More specifically, we are
interested in how the structure of a workflow affects a scheduling's robustness,
that is, its ability to satisfy predetermined constraint in terms of deadline or
cost.
