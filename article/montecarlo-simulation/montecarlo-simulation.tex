...
updating the  times of  upcoming events to  reflect the state change in the
...
simulator, the platform  specification  can  be  done  through  user  code,  as  in
...
amount of computation or communication  to perform. The simulator computes
...
most recently PICS~\cite{pics} which provides end-user point-of-view approach to
simulation.
...
Working  on disjoint low level  components make it easier  to tune
the precision  of the model to  the wanted accuracy or speed trade-off.

PICS, which adopts a strict end-user point-of-view, completely abstracts the
physical platform.
...
we mentioned existing studies relating to IaaS cloud variability. Using the
methodology described in~\cite{LeitnerC16}, based on relative standard deviation
of task runtimes, we find TODO
our platform's variability to be 
...
for ASAP and $P\approx{}5\%$ for AFAP. This perturbation level metric is
analogous to the one used in the later parts of~\cite{pics} and a 10\%
perturbation level matches their findings.
