...
\item ASAP (or \textit{as soon as possible}): schedules the job immediately onto
  ...
  immediately provisions  a new immediately  VM to start  the job on  it.  Hence
  this strategy tries to minimize the makespan of the job batch execution.
...
by $\frac{1}{\sqrt{n}}$~\cite{XX}, where $n$ is the number of samples, but complex applications
