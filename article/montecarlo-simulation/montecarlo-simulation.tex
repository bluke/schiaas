...
to consider  our job runtimes  as the base on  which is applied  a perturbation,
which extent is called the \emph{perturbation level}, denoted $P$ expressed as a
percentage in  the following.  Though  the base is  determined job per  job, the
perturbation level  is set simulation wide.   We set the effective  runtime of a
job $i$ within the $n$\textit{-th} simulation of an \ac{mcs} to:
\[T_i^n = T_i * (1+r_i^n)\]
with $r_i^n$ a value drawn between $[-P;+P]$.
...
$i$ for a given  strategy. This corresponds to the guess of a  user who would be
very  knowledgeable of  the execution  behavior. Regarding  the simulation  wide
perturbation level  $P$, we found  that the best setting  is the average  of the
worst relative deviations for all jobs in the workload:
