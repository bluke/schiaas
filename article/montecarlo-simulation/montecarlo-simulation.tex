...
\begin{abstract}
  ...
  runtimes call for a form of stochastic simulation.
   % -------- to be moved into intro ------
   % which can be achieved
  % through  either complex  calculations  heavily constrained  to some  reference
  % cases   or  through   a  Monte-Carlo   approach  iterating   on  deterministic
  %simulations. 
  ...
  for each level of input precision. We show that \acl{mcs} can be used to
  maintain precise predictions even in cases where inputs show strong
  variability.
\end{abstract}
...
\subsection{Enriched Simulation}

\textbf{(FIXME) Section en chantier.}

The above experimental material is our  basis to evaluate the relevance of using
the \ac{mcs} method described hereafter. As mentioned in introduction, our final
goal is to provide the user with enriched simulation results, based on confidence
intervals rather than the unique and deterministic results the simple simulation
provides given a  set of inputs. Simulation generally introduces  a bias towards
reality because simulation  relies on a modeling of the  processes and platforms
which induces some simplification of the reality. The method we propose intends
to construct intervals in which real runtime observations will fall into, thus
blurring the bias of simulation.

\aclp{mcs} derives the stochastic result of a simulation involving stochastic
inputs by repeated sampling. By repeatedly drawing input values, from \aclp{rv}
modeling the input phenomenons, and simulating them we get results samples that
can be aggregated a stochastic result. \acp{mcs} allow us to use our
deterministic simulator in a stochastic context by using sampling at the cost of
having to run a larger number of simulations. In our context the random inputs
are job runtimes modeled by \aclp{rv}, the simulator SimSchlouder, and the
result samples the makespans obtained by SimSchlouder. 
...
%noticeably variable. The unpredictability of these variation means that a job's
%runtime is best modeled by a random variable which requires a stochastic
%simulation. A \acf{mcs} simulates unpredictable phenomenons by repeatedly
%drawing values from the random variables modeling the phenomenon and computing
%results samples. The results samples can be then aggregated in the final result
%of the \ac{mcs}. Because this result is also a random variable we can establish
%confidence intervals.
=======
The overview  of our  evaluation process  can be sketched  as follows.  One test
consist    in    running    one    application   (among    those    listed    in
section~\ref{sc:setup}) on  the actual platform. We collect the runtimes of the
different jobs and the resulting makespan. These are our observations. Each
observed runtime is then used as a basis to construct a random variable, .

We then run a \ac{mcs}, that is a number of simulations, each job runtimes 

, and we run a  \ac{mcs} based on these observations. It means
the \ac{mcs} repeatedly call simulations with values

% draws its values from a random variable constructed from the
% observation, and produces result samples by running the simu to model the phenomenons in order to get result samples which
...
to  the mean  $\mu$. 
% J'essaye d'introduire la question ici mais ma forumulation est peut etre
% alambiqué
...
In this section we look to establish how precision of the input, the job's
runtimes, affects the precision of the \acp{mcs} output, the width of the 95\%
confidence interval. 
...
around each jobs base runtime were the job's simulated runtimes can be drawn -- 
...
on 6 different real executions of Montage, and done with four
