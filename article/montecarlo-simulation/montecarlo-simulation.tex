...
Simulation has always being corner stone of the study of distributed system.
Because large scale distributed system are such a cost to setup and maintain,
organisation operating them would rather be used for computational purpose
rather than experiment on the functionally. Simulators allowed research on
distributed systems with having to build a dedicated distributed system. Over
the last decade the rise of cloud computing as allowed for the appearance of
multiple simulator dedicated \ac{iaas}.
...
updating the times of upcoming events to refelct the change of state in the
...
widely between simulators. The most common description is as user code that uses
