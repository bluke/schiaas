...
list of  dependencies. Schlouder  uses an  non-dynamic on-line  scheduling where
...
\end{itemize}
% TODO our goal is to highlight exp behaviour
...
completion of the last completed job. The cost is measured as a number of
...
cloud. Table~\ref{tab:nbruns} gives an overview of the executions in the
...
Core to the \acl{mcs} is the accuracy of our job runtimes model. Given a precise
simulator and enough samples the \ac{mcs} will return a precise result, but this
result corresponds to the reality described by the input models. As such for the
result to be a good predictor of the reality, the inputs must be a good
descriptor of reality. An ideal model would present a individual distribution
for every job in the workflow, such distribution would require high level
understanding of the jobs computation, communication, and of the underlying
platform. In real life such modeling is not practical, the underlying platform
is usually not known precisely, and the user might not be deeply familiar with
the application if they as not developed it themselves. For this reason, despite
having ourselves a comprehensive archive, we choose to use a more generic model.

As discussed in Section~\ref{sec:MCS}, for the purpose of our \ac{mcs} we choose
to consider our jobs runtimes as base on which is applied a perturbation. Though
the base is determined job per job, the perturbation level is set simulation
wide. The effective runtime of a job $i$  within the $n$\textit{-th} simulation of an
\ac{mcs} is:
\[T_i^n = T_i * (1+r_i^n)\]
with $r_i^n$ a value drawn between $[-P;+P]$. We call $P$ the perturbation level
and denote it as percentage. 

We take as our base value $T_i$ as the average
effective runtime observed for this job. The simulation wide perturbation level
is average of the worsts relative deviation for every job in the workload: 
...
For OMSSA the perturbation level given by this methods where roughly 10\% for
both strategies. For montage our calculated perturbation level was 20\% when
looking at ASAP executions and 5\% when looking at AFAP executions. We believe
this difference can be partially explained by the low number of montage runs in
the archive. As such we also ran Montage simulation using the 10\% perturbation
...
serves as a reference point when evaluating the strategies behaviour when faced
with perturbations
...
were  the  job's  simulated  runtimes  can  be  drawn  \textendash{}  e.g.~in  a
simulation with  a \pmpc{15} perturbation level  a job with a  100s base runtime
can take any runtime  between 85s and 115s.  These \acp{mcs} will  be based on 6
different real executions of Montage, and done with four different perturbations
levels, \pmpc{5}, \pmpc{10},  \pmpc{15} and, \pmpc{20}.  The  \acp{mcs} are done
