...
Most cloud simulator are based on \ac{des} which allows the simulator to
fast-forward to the next event.\ac{des}s can be simply tested by feeding the
simulator information about a real run to matching simulation and checking that
the simulated chain of event matches the empirical one. The timing of events is
computed by the simulation model. In most cases simulators have a
\emph{bottom-up} approach based on the modeling of low level components
(machines, networks, storage devices), and large scale behaviours emerges
naturally from the interactions of these components. Working on disjointed low
level components make it easier to tune the precision of the model to the wanted
accuracy/speed trade-off. %However the bottom-up approach has a limited
%scalability in cases where the simulated platform becomes to large to
%comfortably simulate. For this reason some simulators offert large scale
%structures.
The models used is usually discriminating factor between two simulators of equal
functionality.

\textbf{FIXME:talk about application specification and platform specification}
...
\textbf{FIXME:Next paragraph needs updating.}
