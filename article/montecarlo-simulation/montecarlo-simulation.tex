...
An alternative  approach is  to extrapolate  the task  durations from  on actual
...
In the context  of cloud computing memory and network contention, virtualisation
...
of possible values for the parameters. And the result of one such simulation is
an \ac{rv} representing the distribution of the results. In this context the
simulation of applications in clouds is analogous to the resolution of
stochastic \acs{dag}, in which the vertices represent the jobs comprising the
...
result would be an \ac{rv} representing the possible the times needed for the
...
that, where tasks runtimes are independent the makespan of successive tasks is a
...
mathematical approach is extremely computationally and its core constraint the
jobs \acp{rv} independence can not be guaranteed in most case. 
...
results by test testing multiple \emph{realisation} in a deterministic fashion.
These realisations are obtained by drawing for every job in the application a
runtime that follows their job's respective \ac{rv}. These realisation each
represent a possible executions of the application and should respect the
distribution of such possibilities. Since within a realisation each job has now
a defined runtime, each realisation can be simulated in a deterministic fashion.
Eventually given enough realisations the distribution of the simulations results
will tend towards the distribution of the equivalent stochastic simulation.
Statistical fitting techniques can then be used to characterise this makespan
\ac{rv} The figure~\ref{fig:mc-process} presents schematically a \ac{mcs}.

Such approach has already been used in the context stochastic
...
repeating simpler deterministic simulations and have realisations scheduled as
...
observed over a number real run and comparing the resulting makespan
...
The left part of figure~\ref{fig:rs} presents schematically the workflow of a
...
payload. The user also select which scheduling strategies Schlouder should
...
As shown on the right side of the figure~\ref{fig:rs} SimSchlouder operates in a
way analogous to Schlouder. It receivers from the user a set of jobs, with their
respective estimated runtimes and dependencies, and a strategy to use. But it
uses SimGrid's cloud interface SchIaaS for provisioning and jobs are events
within the SimGrid's simulated construct of \acp{vm}. Because SimSchlouder is a
simulation it will require some information additional information not required
in the Real Execution Setup. For a real execution the user does not usually know
the cloud specification or configuration that the simulation requires to run.
Most importantly during a real execution the \emph{effective runtime} of a job
is the result of running a job on a \ac{vm} whereas in SimSchlouder it can be
specified as input. This allows simulated jobs to have runtimes different from
the user estimates provided by the user.


\textbf{FIXME: Use fig.2 to show this part}
\begin{comment}
