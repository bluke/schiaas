...
commonalities as  possible.  We  describe hereafter  our experimental  setup. It
consists of two test applications which are  run on one side, on a real platform
with  our scheduler  Schlouder and  on the  other side,  are simulated  with our
simulator SimSchlouder based on SimGrid.  The two environments are sketched side
by side on~Figure \ref{fig:rs}.
...
decisions, whereas integrating the implication of such decision in the already
...
every jobs and providing as results de makespan distribution is referred to as a
\acf{mcs}. The multiple execution of SimSchlouder performed by the \ac{mcs} are
referred to as \emph{simulations}.

Because of the \emph{pay-as-you-go} nature of the cloud we extend our simulation
to two output variable, we will not only observe the makespan computed by any
...
During the development of Schlouder, the cloud borker described in
Section~\ref{sec:work-context}, we performed multiple execution of the
