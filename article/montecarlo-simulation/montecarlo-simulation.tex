...
\begin{comment}
These figures expose the  behaviour
exhibited  by the different strategies. ASAP  appears to finish faster in the
most cases and AFAP is less costly. But  these optimizations come at the cost
of instability on the dimension they  aim to  optimize. ASAP uses  a high number
of \ac{btu}s  but this number is stable, whereas AFAP uses a lower number of
\ac{btu}s  but this number of \ac{btu}s is  more variable.  When looking at the
makespan, we take note of how the different strategies affect the skewness of
the overall distribution. The  positive skew seen on  ASAP suggests that even
though  the strategy globally produces  shorter makespan, perturbations are more
likely  to extend the makespan, whereas AFAP  makespans are more spread out and
skews negatively.
...
We ran 500 iteration \acp{mcs} for all for strategy/application group using the
...
distribution of a specific metric for a given applications.

When OMSSA results distribution, the first to graphs. The distribution of
...
shows a very stable BTU count which is match perfectly by the simulation.

To provide a more quantitative approach when discussing makespan distribution we
decide to fit simulated distribution to a normal distribution. This will allow
us to build confidence intervals that can be checked against real observation.
...
Table~\ref{tab:fit} presents that capture rate obtained by each interval
computed after normal fitting. The capture rate expresses the number of observed
real makespan fall withing a given \ac{ci} relative to the total number of real
observation. Since improving the capture rate is always possible at the cost of
...
Still on OMSSA we can see that our normal fitting in not perfect our \acp{ci}
appear of by 5\% with the 95\% \ac{ci} capturing only 90\% real observations.
...
Here again the graph show great coverage. Distribution can not be significantly
...
have a good look at distribution, but the platform on which these experiment
where run does not exist at this time preventing us from obtaining new real
executions. We kept this limited Montage run as to check that the results
...
characterisation of the results less reliant on the normal distribution which
...
real makespans over all runs. These \ac{ci} subfigures only presents ASAP
...
bound at 10\%. This impacts the capture rate of the simulation form 90\%
...
This results is two-fold. First this demonstrate that the perturbation level can
...
interval compacity should you statistical methods to build higher rate \acp{ci},
like in our case the 99\% normal distribution \ac{ci}.

Secondly this results show how an \ac{mcs} can be used to exhibit strategy
...
level. In the case of ASAP these behaviour could probably have been predicted by
