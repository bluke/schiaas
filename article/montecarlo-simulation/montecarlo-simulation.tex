...
used by the simulator to compute the jobs end date without relying solely on the
estimated runtimes. 
...
model the input \acp{rv} representing the jobs' runtimes.
...
We looked at every difference between simulations and reality, 
...
average of all observed runtimes for job $j$, and $p$ a value drawn from the 
