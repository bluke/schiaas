...
to two output variables: we will not only observe the makespan computed by any
...
distribution observed over a number real runs and comparing the resulting
...
number of observation in those group is uneven due to the changing nature of the
...
counts for all the selectd real runs separated by application and strategy.
Makespan distribution is represented using an density curve, by definition these
curves have an integral equal to 1, whereas as a discrete value \ac{btu} count
is presented using a distribution bar graph.
...
Improving on these results would require
better inputs models and a better characterisation of the results less reliant
on the normal distribution that cannot reflect skewness. Although such
improvements would be possible in our cases this is not the case  
