...
%\Large{
%- Imprécisions sur les input -> effet sur le Makespan 
%- On étudie la variablilité du Makespan en focntion de la variabilité des
%entrées. Subsdiaire : à quelle vitesse ce résultat est obtenu par MCS
%}
...
%clusters.  For every base set we generate 500 realizations.
%Every  realization is then simulated by SimSchlouder.
%In order  to study the  properties like convergence speed  we opted to  keep the
%results of each individual simulation indexed by base set, and perturbation level.
...
\section{Effect of input perturbation on the result of a \acl{mcs}}
...
\section{\acl{mcs} convergence and confidence}
