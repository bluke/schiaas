...
Trustworthiness of simulation of variable environments is a major problem when
simulating cloud. If the result of the simulation is one of many its relevance
is immediately compromised. Users need results that contains actionable action.
As such providing a distribution will of possible results is a better solution.
...
extensive results provides by stochastic simulations. This approach garantee
that our simulation respects the scheduling and provisioning decision observed
in real life, as the \ac{des} can easily be programmed to reproduces such
decision, whereas integrating the implication of such decision in the already
complex computation of the numerical results of a stochastic would be
impossible.

In this section we will first discuss the particulars of the Monte Carlo
...
We now  describe our contribution consisting  in an automated system  to provide
...
$i \in \[1,\dots,n\]n$,  the user provides a runtime estimate  $T_i$, from which
the system computes a \ac{rv} $R_i$ (to be explained below).

One MCS-step then consists in 
\begin{itemize}
\item making one realization of the runtime  for each job $i$, that is randomly
  drawing a runtime value from the associated $R_i$ to obtain a runtime $T_i$,
\item and once runtimes have been realized for all jobs, proceed to a simulation
  of the execution with these runtime values to obtain a makespan $M$.
\end{itemize}
The whole MCS process  consists in repeating this step a  large number of times
(500 times  on the  figure). From the  set of makespans  $M^k$ obtained  at each
step, we compute  a statistical distribution of the makespan  as a final \ac{rv}
noted $M_S$.
