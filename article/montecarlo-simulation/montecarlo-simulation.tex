...
First we  found that for  our purposes the  \ac{mcs} only requires  a relatively
small number of  deterministic simulations to be successful. When  used in other
domains  such  as  physics  \acp{mcs}  are  often  used  with  up  from  20\,000
deterministic simulations,  but the  size of the  input distribution  is usually
much  larger.  The  result  presented  in this  paper  are  obtained  using  500
simulations per \ac{mcs}.  By keeping  the results of the individual simulations
ordered, we are  able to determine that by the  hundredth simulation the average
makespan is  already precise to the  second, with its value  hardly changing by
the addition of the 400 next simulations. The standard deviation used to compute
the width  of the makespan interval is less stable, but  still strongly shows
diminishing returns past the 100th simulation mark.
