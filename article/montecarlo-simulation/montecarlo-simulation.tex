...
servers as long as the price can be afforded. However, for applications composed
of a set of jobs, that can be  parallel or dependent on some others (generally a
\ac{dag}  of  jobs) it  quickly  becomes  difficult  to manually  provision  the
...
time  and cost  of such  complex  workloads, in  which the  scheduling plays  an
...
an opaque  fashion, the exact nature  of the underlying platforms  being unknown
and their hardware subject to evolution as operators complete their data-centers
...
As in almost all batch scheduler systems, the job description provides a runtime
estimate  called  \emph{user   estimate}.  In  case  of  a   workflow,  the  job
dependencies are  also provided.   Schlouder uses just-in-time  scheduling where
jobs are assigned  to \acp{vm} as soon as all  their dependencies are satisfied.
We call \emph{strategy} the heuristic used by Schlouder to make provisioning and
...
change the initial scheduling decision.
...
The user  sends to Schlouder a  list of jobs. For  each job the user  provides a
runtime  estimate (as  in  almost  all batch  scheduler  systems),  the list  of
depedencies and the scheduling strategy.   Schlouder computes the scheduling and
resource provisioning  decisions and  communicates its  requests to  the cloud's
frontend, denoted Controller.  Once the \acp{vm} are booted Schlouder sends each
job to its assigned machine.   Schlouder tracks jobs progression, completing the
schedule  as dependencies  are resolved,  provisioning \acp{vm}  as needed,  and
shutting down unneeded \acp{vm} as their billing cycle close.
...
for scheduling purposes. For a better control of the simulation, SimSchlouder
...
In this section we propose one such extended simulation by instrumenting a
\ac{des}, SimSchlouder, as a stochastic simulator using the method of the
\ac{mcs}. This solution combines the relative simplicity of a \ac{des} with the
...
In this paper the simulation done within each MCS-iteration is performed by
SimSchlouder.
...
enough for the \ac{mcs} to predict 90\% of real runs.
...
strategies. For Montage  our calculated perturbation level is  $P\approx{}20\%$ when looking
at ASAP  executions and $P\approx{}5\%$ when  looking at AFAP  executions.

These models offer the best guess as to the job's variability. This allows us to
evaluate our \ac{mcs} without the interference of estimation mistakes.
...
We ran a 500 iterations \ac{mcs} for every strategy-application combination using
...
from fig.~\ref{fig:realbrs}, as non-filled curves. On the \ac{btu} count graphs real
...
range of observed makespans. But the simulated makespans do not exactly share
the same distribution as the real observed executions.
The OMSSA BTU count distributions offers
...
distribution do not completely match the real one.

After this visual inspection of the results we now examine them thought
...
distribution which has a finite variance, we consider the possible application
Central Limit Theorem. Fitting to a normal distribution gives us an average
makespan $\mu{}$, and a standard deviation $\sigma{}$. These can be used to
build \acp{ci}. For the normal distribution the 95\% \ac{ci} is defined as
$[\mu{}-2\sigma{},\mu{}+2\sigma{}]$ and the 99\% \ac{ci} as
$[\mu{}-3\sigma{},\mu{}+3\sigma{}]$.

The capture rate expresses the number of observed real makespans that fall
withing a given \ac{ci} relative to the total number of real observations.
...
\begin{comment}
For example
the first line of table~\ref{tab:fit} presents the results for OMSSA using the
ASAP strategy, this this case the 95\% \ac{ci} spanned 419s, from 12797s to
13216s, for an average makespan of 13006s. Therefore the relative size is given
as 3\%.
\end{comment}

On OMSSA the \ac{mcs} captures 90\% of real observed makespans. The divergence
between the capture rate and the \ac{ci} expected capture rate is to to the fact
that real observation distribution does not follow a perfect normal distribution. Using a
higher confidence intervals improves the capture rate up to 98\%.
When simulating Montage the \ac{mcs} achieves a capture rate of 100\% for any
\ac{ci}.
...
\end{comment}
Further more this \ac{mcs} offers plenty of opportunities other than runtime and
cost predictions.
...
\begin{figure*}
	...
	the boxes represent the 95\% confidence interval resulting from the \acs{mcs},
...
The first subfigure presents the \acp{ci} for OMSSA using the ASAP strategy at
