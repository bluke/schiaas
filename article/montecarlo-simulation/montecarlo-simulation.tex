...
corresponding to a given job, denoted as superscripts indices. A realisation is
a set of $n$ runtimes that where drawn according to each jobs respective
\ac{rv}.  In figure~\ref{fig:mc-process} we denote realisation number as
subscript indices. Each of this realisation is simulated independently,
...
$i  \in\,[1;n]$, we provide  a runtime distribution $T^i$ following the method
...
\item making one realization of the runtime  for each job $i$, that is randomly
  drawing a runtime value from the associated $T^i$ to obtain a runtime $t^i$,
...
\end{itemize}
The whole \ac{mcs} process  consists in repeating this step a  large number of times
(500 times  on the  figure). From the  set of makespans  $m_k$ obtained  at each
...
\textbf{FIXME: talk about fitting \& qualitative vs quantitative approach here
or in next section~?}
