...
results. But creating such model would not be possible in more common use cases
...
thought a series of script created to automate large numbers of simulations. The
...
necessary number of simulation input file, with task runtimes randomised
...
\ac{mcs} output file. This process is supple enough accommodate other simulator
and models as long as the user can provide a command to generate input files,
and one to parse simulation outputs. 
...
traces, using scheduling and provisioning heuristics that  aim to optimize
...
scheduling heuristics  given  an appropriate perturbation  level. We now aim
