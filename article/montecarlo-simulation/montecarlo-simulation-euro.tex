...
\begin{comment}
This model for the runtimes \acp{rv} uses a task's estimated runtime and a global
perturbation  level  for  all  tasks.  The perturbation  level is denoted as $P$
and is expressed as a percentage of estimates. It is used  to build a range in 
...
given task. As such $t_j$, the drawn time of task $j$, can be written as~:
\begin{equation}
...
\begin{equation}
	...
	\label{eq:r}
\end{equation}
where $r_j^n$ is the $n$th runtime observation for task $j$, $\bar{R_j}$ is the
average of all observed runtimes for task $j$, and $p$ a value drawn from the 
