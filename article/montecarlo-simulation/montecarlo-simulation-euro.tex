...
\author{Luke~Bertot
	\and Stéphane~Genaud\orcidID{0000-0003-0065-8083} 
	\and Julien~Gossa}
\institute{Icube-ICPS --- UMR 7357, Univeristé de Strasbourg, CNRS\\
		P\^ole API, 300 Blvd S. Brant, 67400 Illkirch\\
		\url{lbertot@unistra.fr}, \url{genaud@unistra.fr}, \url{gossa@unistra.fr}
	}

\authorrunning{L. Bertot et al.}
...
Second, it  is useful  for parallelizing tasks to  achieve a  shorter makespan
...
by multiple factors.  Firstly \ac{iaas} operates  in an opaque fashion: the exact
...
one   of   the   most   comprehensive   and  recent   surveys  by   Leitner   et
...
of the system and on the phenomena it can capture.  In our work, we rely on the
...
, and the scheduler are presented.  The stochastic  framework we propose is then
presented in section~\ref{sec:enriched-sim} and is evaluated in 
section~\ref{sec:eval}. We discuss the limits of this approach in 
section~\ref{sec:disc}. %the possible
...
events can  be the  start (or  end) of computations  or communications.  The
...
strong  focus  on  energy  consumption  of cloud's  network  apparatus  using  a
...
projects is iCanCloud~\cite{iCanCloud} proposed  to address scalability issues
encountered  with  CloudSim  (written  in  Java) for  the  simulation  of  large
use-cases.  Most recently,  PICS~\cite{pics} has  been proposed to evaluate 
specifically the simulation of public clouds. The configuration of the simulator uses
...
task's respective \ac{rv} for every  task in the application. This allows one to
...
\ac{rv}. \acp{mcs} permits non-independent \ac{rv} and dynamic scheduling.
This approach was first suggested in \cite{Slyke63}  for stochastic  PERT graphs.   
...
one execution, Tang et al.~\cite{Tang11} proposed, a modification of the well-known
...
evaluated the  impact of  this  variability  on  the makespan.   More  recently,
...
The study  conducted in this  paper is built  upon a real comparison between
...
setup described hereafter consists of two test applications which, on one hand,
are run on a real platform with our scheduler Schlouder, and on the other hand 
are simulated with our simulator SimSchlouder based on SimGrid.
...
decisions. 
%
The  schedulig problem  in IaaS  clouds is  a bi-ojective  optimization problem,
taking  into   account  the   rental  cost  of   resources  and   the  execution
makespan.  Schlouder's requests  users  to  choose a  strategy  that favors  one
objective  or the  other. The  scheduling  and provisioning  decisions are  then
controlled accordingly by specific \emph{heuristics}. In this paper, we used the
two following heuristics:
\begin{itemize}
\item ASAP (\textit{as soon as possible}) schedules each task onto
  ...
  This heuristic minimizes the makespan.
...
\item AFAP (\textit{as full as  possible}) schedules each task onto
  ...
  of \ac{btu}), or provisions a new VM otherwise. This heuristic minimizes cost
...
the same scheduling heuristics as Schlouder. It uses SimGrid as its core
...
before choosing an heuristic for a real run.
%
SimSchlouder  shares with Schlouder a common subset of inputs, including
the same tasks description and heuristic. Whereas Schlouder operates on a
...
\begin{table} \centering \caption{Overview of archived
	...
	\begin{tabular}{llrcc} \toprule
		Application&Heuristic&\#runs&BTU count&Makespan ($s$)\\
		\midrule 
...
comparable runs are grouped by application and heuristic.  Fig.~\ref{fig:realbrs}
...
user with  a range of  possible makespans. 
%----
The \ac{mcs} requires a runtime \ac{rv} for every task in the application. These
\acp{rv} form the input model. Although precise models will yield more exact
results, creating such models would not be possible in more common use-cases
where a backlog of real observations is not available.
%----
...
This model for the runtimes \ac{rv} uses a single expected runtime per task and
a global perturbation level for all tasks. This model uses uniform distributions 
($\mathcal{U}$). These \acp{rv} are centered on the expected runtime of the task 
they represent. The relative spread of these distributions is defined by the 
\emph{perturbation level} $P$, which is the same for every task.  If we assume 
$P$ can summarize the variability of the whole system, a central question is how
should $P$ and the expected runtimes be chosen to assess the validity of the 
\ac{mcs}. To this end, we assume a good guess for an expected runtime is the
average of all effective runtimes, $\bar{r_j}$ for a given task $j$. 
As such the runtime distribution's \ac{rv} $T_j$ for task $j$ is: 
\begin{equation} 
...
heuristics. For Montage  our calculated perturbation level is  $P\approx{}20\%$
...
\paragraph{Simulation execution.} The execution of an \ac{mcs} is implemented
through a series of scripts created to automate large numbers of simulations. The
simulation driver first passes an application template, including dependencies
and task expected runtimes, to a generator script. The generator creates the
necessary number of simulation input files, with task runtimes randomised
following the input model. The driver script can then execute an instance of
SimSchlouder for every input file, sequentially or concurrently. Once all the
instances of SimSchlouder have been executed, the result are aggregated in the
\ac{mcs} output file. This process is supple enough to accommodate other 
simulator and models as long as the user can provide a command to generate input
files and another to parse simulation outputs. 
...
\begin{figure}[bt]
	...
	\label{fig:fit}
\end{figure}
We ran a 500-iteration \ac{mcs} for every heuristic $\times$ application group using
...
%
These graphs show the simulation results cover the same ranges as the real
...
\begin{table}[b]
	...
	\begin{tabular}{llccc}
		\toprule
		Application&Heuristic~&\multicolumn{2}{c}{~Makespan (Size of
		CI)~}&~BTU\\
                           &         & CI 95\% & CI 99\% &\\
		\midrule
...
illustrate one such application. Then we will discuss limitations we have
...
section  we will  focus  on simulation  of makespans  using  the ASAP heuristic.
...
%
Secondly, this result shows that \ac{mcs}s can be used to exhibit heuristic
behaviours. This upwards shift of the \ac{ci} shows that ASAP, an heuristic
...
remains capable of low makespans. 
%--
This can be explained by the scheduler's 
behaviour and the workflow's nature. In workflows the makespan depends only on 
execution of tasks in the critical path, and remains unaffected by variability 
of tasks outside the path. This is compounded by the just-in-time scheduling
used in Schlouder, later scheduling decisions take into account the tasks'
deviation from their expected runtimes. 
%--
%This behaviour is explained by the nature of 
%workflows, in which tasks have to wait for their last dependencies resolution 
...
weaknesses of any heuristic, regardless of complexity.
...
influences the  input variable distribution,  we are currently studying its
relationship with the number of required MCS-iterations.

\section{Conclusion}
Predicting  the execution  behaviour of complex  workloads in  the  cloud is  an
important challenge. While a number of works have proposed model-driven
...
task runtimes. Second, we apply our model in  a real setting, on two different
applications, for which we have collected  execution traces.  At  the light of
these empirical observations,  our study shows that the proposed model could
capture over 90\% of the observed makespans for  all  combinations  of
application   and scheduling heuristics  given  an appropriate perturbation
level. We now aim to test our simulator  on more use-cases and platforms.  In
particular as a number of studies on public clouds have reported variability
levels similar to our platform (\cite{LeitnerC16,pics}), we intend to reproduce
these results on public clouds.
