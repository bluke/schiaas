...
and effective runtimes. They were initially used all along the development of 
Schlouder and then to properly tune SimSchlouder in order to make the simulation
as accurate as possible. As a result, for the execution used in this paper, 
simulations done with SimSchlouder are precise to the second on the makespan and
...
comparable runs are grouped in \{application, strategy\} groups.
Figure~\ref{fig:realbrs} presents the distribution of resulting makespans and
\ac{btu} counts. For OMSSA, the makespan varies from 12811s to 13488s with a BTU
count of 40 when using ASAP, whereas when using AFAP the makespan varies from
13564s to 14172s with a BTU count varying from 33 to 36. For Montage, makespan
goes from 1478s to 1554s with a BTU of 10 for ASAP and from 2833s to 2837s with
a BTU count of 1 for AFAP.
