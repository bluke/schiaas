...
largely extended  to tackle  new fields of  distributed computing.   Its layered
design%
\textbf{FIXME:SimGrid Layers depicted referenced here} %
aims to offer  specialized interfaces to properly model applications  in a given
field, e.g peer-to-peer systems~\cite{QuinsonRT12} or high-performance computing
with SMPI~\cite{DegommeLMQSS17}.  All interfaces rely on the \ac{des} simulation
core engine.  
%
For our study, we developped a new interface upon MSG %
\textbf{FIXME: dire ce qu'est MSG en un mot et faire référence à une figure.}~%
dedicated to model \ac{iaas} operations and called SchIaaS.
%
While SimGrid  implements hypervisor level  functionalities such as  starting or
stoping a \ac{vm}, SchIaaS implements  cloud level functionalities (e.g. run and
terminate instances%
\textbf{FIXME: définir instance.}~%
It offers the following instance management fuctions:
\begin{itemize}
\item run, terminate, suspend, resume and describe instances
\item description of available resources
\item image and instance types management
\item VM placement on the clusters
\item boot and other VM life-cycle processes
\item cloud storage management.
\end{itemize}
