...
\author{Julien Gossa, Luke Bertot, Stéphane Genaud, Stéphane Genaud}% A voir l'ordre. 
...
migrations concurrentes (quantité sur l'axe \textit{value}), qu'une intervale de
100  secondes ne  présente aucun  intérêt  en terme  d'optimisation, mais  qu'un
intervalle  de 10  secondes optimise  raisonablement la  plateforme, en  évitant
quasiment toute migration  concurrente.  Il est imporant  de remarquer également
que  ces   graphiques  ont  les   travers  inévitables  des   données  produites
automatiquement   :  titres   abscons,  labels   génériques,  et   échelles  par
défaut. L'utilisateur souhaitant  raffiner ces données devra  adapter le fichier
\texttt{template.R}.
...
L'automatisation des simulation et des observations permet non
seulement de s'assurer que rien n'échappe à l'expérimentateur, mais rend aussi
...
l'ensemble d'une archive de 273 exécutions réelles, chacune dans 5 conditions 
exéprimentales différentes, soit plus de 1300 simulations à observer. Nous 
l'utilisons également pour des simulations de
Monte Carlo, en procédant à des lots de 500 simulations afin d'approcher 
l'évaluation des performances d'une application dont les temps d'exécution sont 
difficilement modélisables.
Des études par la simulation de tel volumes sont pratiquement impossibles à 
mener des solutions n'intégrant pas tous le processus d'étude de bout-en-bout.
