...
Afin d'illustrer concrètement le fonctionnement de notre \textit{framework},
nous utiliserons l'exemple d'une étude de différentes stratégies de placement de VM sur PM.
Ce problème, parfois appelé \textit{VM Packing}, peut poursuivre plusieurs objectifs.
Un des plus populaires est de réduire le nombre de machines physiques utilisées 
dans un soucis de réduction de la consommation électrique. 
Pour ce faire, de nombreux algorithmes permettent d'établir 
des plans de migration reconsolidant les VM sur moins de PM.
Pour un administrateur, le problème est pragmatiquement de configurer la fréquence 
de ces reconsolidation pour éviter de surcharger sa plateforme avec trop de migrations.

Nous allons montrer comment notre solution permet de facilement évaluer
le gain et l'impact des reconsolidations en fonction de leur 
fréquence. 

\subsection{Setup}

Le conception d'un systeme systeme d'ordonnancement se fait par l'implementation de
l'interface \texttt{ComputeScheduler}, et de la méthode \texttt{schedule} qui indique
l'hôte à utiliser pour déployer une VM d'un type donné. C'est le seul 
développement que devra faire un expérimentateur souhaitant tester son propre 
algorithme.
Nous utiliserons \texttt{SimpleReconfigurator} qui tente d'optimiser 
le placement des VMs en migrant une VM toutes les $x$ secondes.
Nous allons comparer les résultats avec $x=0$, $x=10$ et $x=100$.

Ce problème est purement administrateur, seule compte la charge en terme de 
nombre de VMs et de CPU de ces VMs, peu importe les applications qui y sont exécutées.
Nous allons donc utiliser l'application \emph{LoadInjector}, dont la fonction 
principale est \texttt{loadinjector.SimpleInjection} et qui charge 
simplement un injecteur dans la simulation.
Nous allons utiliser \texttt{SinInjector} qui injecte une charge de calcul 
sinusoïdale sur un nombre de VMs suivant également une sinusoïde.
...
\end{verbatim}
\begin{description}
	\item[platform.xml] décrit l'infrastructure matérielle pour SimGrid
	\item[deploy.xml] instancie les processus SimGrid (typiquement vide 
dans notre cas)
	\item[cloud.xml] décrit le cloud, en particulier l'ordonnanceur 
	\item[injector.xml] sélectionne et paramètre les injecteurs à utiliser
\end{description}
 
Dans notre cas, il y aura donc trois fichiers de configuration du cloud, 
correspondant aux trois intervales testés :\texttt{cloud-reconsolidator0.xml}, 
...
variables à observer, et si nécessaire les pré- ou post- traitements requis. 
