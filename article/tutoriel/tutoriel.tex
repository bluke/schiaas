...
\node[level] (Lab) {Lab};
\node[module2,right=of Lab] (labpy) {Pilotage des 
simulations};
\node[module2,right=of labpy] (TU) {Analyse des 
observations};
...
\node[module3,right=of App] (LI) {LoadInjector};
\node[module3,right=of LI] (SS) {SimSchlouder};
\node[module3,right=of SS] () {...};

\node[level,below=of App] (SCHIaaS) {SCHIaaS};
\node[module1,right=of SCHIaaS] (API) 
{API de \textit{cloud-kit}};
\node[level,below=of SCHIaaS] (SCHIaaS2) {};
\node[module3,right=of SCHIaaS2] (CE) {Compute Engine};
\node[module3,right=of CE] (SE) {Storage Engine};
\node[module3,right=of SE] (NE) {Network Engine};
\begin{pgfonlayer}{background}
\node[moduleset, fit=(API) (CE) (SE) (NE)] {};
\end{pgfonlayer}

\node[level,below=of SCHIaaS2] (SG) {SimGrid};
\node[module1,right=of SG] (SG) {MSG};
