...
"""
Thanks to brentp: http://hackmap.blogspot.fr/2008/11/python-interval-tree.html
and Wikipedia: http://en.wikipedia.org/wiki/Interval_tree#Construction
"""
...
class IntervalTree(object):
    ...
    def find(self, interval):
        ...
        if self.intervals and interval.stop >= self.intervals[0].start:
            overlapping = [i for i in self.intervals if i.stop > interval.start
                                                    and i.start <= interval.stop]
        else:
            overlapping = []

        if self.left and interval.start <= self.center:
            overlapping += self.left.find(interval)

        if self.right and interval.stop >= self.center:
...
class Interval(object):
    ...
    def __init__(self, start, stop):
        if start > stop:
				    raise ValueError('stop must be greater or equal than start')
        self.start = start
        self.stop  = stop
    def __repr__(self):
