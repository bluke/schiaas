...
public abstract class AStrategy {
	...
	public void execute() throws SimSchloudException {		
		...
		for (Integer l : tasks.keySet()) { 
			int iTask = 0;
			while (iTask<tasks.get(l).size() && (SchloudController.idleNodesCount!=0 || Compute.describeAvailability(SchloudController.cloud.name)>0) ) {
				...
				if (node != null) {
					Msg.info(task.getName() + " selected node : " + node.instance.getName());
